...
def save_ppm(filename, image):
    ...
    write_pixels(image, height, out_file)

    # Provided - gotta close the file, skipping this is bad ok?
    out_file.close()


def write_pixels(image, height, out_file):
    for pixel_index in range(height):
        for column in image:
            print(create_ppm_string(column, pixel_index), file=out_file)


def create_ppm_string(column, pixel_index):
    return '{} {} {}'.format(column[pixel_index][0], column[pixel_index][1], column[pixel_index][2])
